...
See https://pinia-colada.esm.dev/#TODO`
  ...
  const useDataLoader = () => {
    ...
    const useDataLoaderResult = {
      ...
      error,
      isLoading,
      reload: (to = router.currentRoute.value) => router[_dataloaders.APP_KEY].runWithContext(
        () => load(to, router, void 0, void 0, true)
      ).then(() => entry.commit(to)),
      // pinia colada
      refetch: (to = router.currentRoute.value) => router[_dataloaders.APP_KEY].runWithContext(
        () => load(to, router, void 0, void 0, true)
      ).then(() => (entry.commit(to), entry.ext.state.value)),
      refresh: (to = router.currentRoute.value) => router[_dataloaders.APP_KEY].runWithContext(() => load(to, router)).then(
