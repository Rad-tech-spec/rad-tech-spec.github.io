...
function toExports(imports, fileDir, includeType = false) {
  const map = toImportModuleMap(imports, includeType, (i) => {
    ...
    return i.with ? `'${from}' with ${stringifyWith(i.with)}` : `'${from}'`;
  });
  return Object.entries(map).flatMap(([from, imports2]) => {
    const entries = [];
...
function stripFileExtension(path) {
  return isFilePath(path) ? path.replace(/\.[a-z]+$/i, "") : path;
}
function toTypeDeclarationItems(imports, options) {
  const map = toImportModuleMap(imports, true, (i) => {
    ...
    return i.with ? `import('${from}', { with: ${stringifyWith(i.with)} })` : `import('${from}')`;
  });
  return Object.entries(map).flatMap(([from, imports2]) => {
    const entries = [];
    const filtered = Array.from(imports2).filter((i) => {
...
function toImportModuleMap(imports, includeType = false, getKey) {
  const map = {};
  for (const _import of imports) {
    if (_import.type && !includeType)
      continue;
    const key = getKey?.(_import) || _import.from;
    map[key] ??= /* @__PURE__ */ new Set();
    map[key].add(_import);
  }
  return map;
}
